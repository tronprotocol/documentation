...
# 6 用户地址生成过程
## 6.1 算法描述
首先产生密钥对，取公钥，仅包含x，y坐标的64字节的byte数组。对公钥做sha3-256的hash运算。取其最后20字节。地址在前面补41，得到地址的原始格式。长度为21字节。做两次sha256计算，取其前4字节得到校验码。将校验码附加在地址的原始格式后面，做base58编码，得到base58check格式的地址。地址编码后以T开头，长度34字节。
