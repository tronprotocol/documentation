...
但是过多无关紧要的交易仍然会消耗过多的带宽和CPU资源时，导致系统阻塞，影响正常交易的处理确认。
为了保持交易的相对公平，波场网络引入了Bandwidth point 和 Energy 两种资源。
其中带宽消耗的是Bandwidth Point，而磁盘、内存、CPU消耗的是Energy。   
**注意** 
...
智能合约的创建和运行需要消耗磁盘，内存和CPU，为使用TRON网络中的这些资源，必须消耗一定量的Energy。消耗Energy的大小，跟虚拟机的具体指令有关，
包括指令执行过程中使用的磁盘、内存和CPU，其中每个指令占用CPU消耗Energy大小是固定值。
...
例如，假设当前网络中为了Energy而冻结的TRX总量为1000_000_000TRX，一个账户冻结了1000TRX，占总冻结量的百万分之一，即50_000Energy。    
如果一条合约执行需要花费调用者500Energy，那么用户可以触发100次该合约。

**注意** 
- 由于网络中总冻结量以及账户的冻结量随时可能发生变化，因此账户拥有的Energy资源不是固定值。 
